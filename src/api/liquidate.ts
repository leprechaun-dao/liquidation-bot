  ...
  try {
    ...
    switch (action) {
      case 'scan':
        ...
        const limit = Number(request.query.limit || String(botConfig.batchSize));
        
        logger.info(`API request to scan for liquidatable positions (skip: ${skip}, limit: ${limit})`);
        
        const profitableLiquidations = await scanner.scanForProfitableLiquidations(
          skip,
          limit
        );
      ...
      case 'liquidate':
        ...
        logger.info(`API request to liquidate position ${positionId}`);
        
        const result = await liquidator.executeLiquidation(BigInt(positionId as string));
      ...
      case 'scan-and-liquidate':
        ...
        logger.info('API request to scan and liquidate profitable positions');
        
        const results = await liquidator.scanAndExecuteLiquidations();
